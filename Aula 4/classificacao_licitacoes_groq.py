"""
Pipeline local de classificação de licitações via Groq.
Execução: python classificacao_licitacoes_groq.py
...
def classificar_com_llm(objetos):
    ...
    logger.info(f"Classificando {len(objetos)} licitações | Provider: groq | Modelo: {GROQ_MODEL}")
